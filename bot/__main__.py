...
def start(update, context):
    start_string = f'''
Kono botto wa, gūguru doraibu e no subete no rinku o mirāringu dekimasu!
Taipu /{BotCommands.HelpCommand} Shiyō kanōna komando no risuto o shutoku suru ni wa
'''
    buttons = button_build.ButtonMaker()
    buttons.buildbutton("IndiAnime Base", "https://indianimebase")
    buttons.buildbutton("Discussion Group", "https://t.me/indianimein")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    LOGGER.info('UID: {} - UN: {} - MSG: {}'.format(update.message.chat.id, update.message.chat.username, update.message.text))
    uptime = get_readable_time((time.time() - botStartTime))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        if update.message.chat.type == "private" :
            sendMessage(f"Watashi wa otto ga ikite imasu 🙂\nSince: <code>{uptime}</code>", context.bot, update)
        else :
            sendMarkup(start_string, context.bot, update, reply_markup)
    else :
        sendMessage(f"Ottotto! Kyoka sa reta yūzāde wa arimasen.", context.bot, update)


def restart(update, context):
    restart_message = sendMessage("Sakidō shimasu, omachikudasai!", context.bot, update)
